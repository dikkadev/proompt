...
export function MainSidebar({ 
  ...
  const groupedSnippetsByTag = useMemo(() => {
    return snippets.reduce((acc, snippet) => {
  ...
  const sortedSnippetTags = useMemo(() => {
    return Object.keys(groupedSnippetsByTag).sort((a, b) => 
      groupedSnippetsByTag[b].length - groupedSnippetsByTag[a].length
    );
  }, [groupedSnippetsByTag]);
  ...
  const handleToggleAllSnippets = () => {
    const allCategories = snippetViewMode === 'categories' ? sortedSnippetCategories : sortedSnippetTags;
    if (expandedSnippetCategories.size === allCategories.length) {
      // If all are expanded, collapse all
      setExpandedSnippetCategories(new Set());
    } else {
      // Otherwise, expand all
      setExpandedSnippetCategories(new Set(allCategories));
    }
  };
  ...
  const filteredSnippets = useMemo(() => {
    ...
    return snippets.filter(snippet =>
      ...
      (snippet.tags && snippet.tags.some((tag: string) => tag.toLowerCase().includes(query.toLowerCase()))) ||
      (snippet.category && snippet.category.toLowerCase().includes(query.toLowerCase()))
    );
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild className="cursor-pointer">
                        <Button
                          ...
                          className="h-6 w-6 p-0"
                        >
                          {expandedSnippetCategories.size === (snippetViewMode === 'categories' ? sortedSnippetCategories.length : sortedSnippetTags.length) ? <Minimize className="h-3 w-3" /> : <Maximize className="h-3 w-3" />}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        {expandedSnippetCategories.size === (snippetViewMode === 'categories' ? sortedSnippetCategories.length : sortedSnippetTags.length) ? 'Collapse all snippets' : 'Expand all snippets'}
                      </TooltipContent>
                    </Tooltip>
                    <Separator orientation="vertical" className="h-4 mx-1" />
                    <Tooltip>
                ...
                {combinedQuery || snippetQuery ? (
                  ...
                  <div className="space-y-1 px-1">
                    {snippetViewMode === 'categories' ? (
                      ...
                      sortedSnippetTags.length > 0 ? (
                        sortedSnippetTags.map(tag => (
                          <Collapsible 
                            key={tag} 
                            open={expandedSnippetCategories.has(tag) || allSnippetsExpanded || sortedSnippetTags.length <= AUTO_EXPAND_THRESHOLD} 
                            onOpenChange={() => toggleSnippetCategory(tag)}
                            className="my-px cursor-pointer"
                          >
                            <CollapsibleTrigger asChild>
                              <Button variant="ghost" className="w-full justify-between font-normal h-7 px-1 py-1 text-sm">
                                <span className="flex items-center gap-1 min-w-0">
                                  ...
                                  <span className="flex-1 overflow-hidden whitespace-nowrap cursor-pointer">{tag}</span>
                                </span>
                                <span className="flex items-center gap-1">
                                  <Badge variant="outline" className="ml-1 text-xs px-0 py-0 flex items-center justify-center border-transparent bg-transparent text-muted-foreground shrink-0">
                                    {groupedSnippetsByTag[tag].length}
                                  </Badge>
                                  {expandedSnippetCategories.has(tag) ? <ChevronDown className="h-3 w-3 text-muted-foreground" /> : <ChevronRight className="h-3 w-3 text-muted-foreground" />}
                                </span>
                              </Button>
                            </CollapsibleTrigger>
                            <CollapsibleContent className="space-y-1 p-0">
                              {groupedSnippetsByTag[tag].map(snippet => (
                                <SnippetCard key={`${snippet.id}-${tag}`} snippet={snippet} onInsert={onSnippetInsert} className="ml-1" />
                              ))}
                            </CollapsibleContent>
                          </Collapsible>
                        ))
                      ) : (
                        <div className="text-sm text-muted-foreground py-1 px-1">No snippets with tags available.</div>
                      )
                    )}
                  </div>
                )}
